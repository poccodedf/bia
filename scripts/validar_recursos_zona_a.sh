...
security_group_id=$(aws ec2 describe-security-groups --group-names "http-bia-sg" --query "SecurityGroups[0].GroupId" --output text 2>/dev/null)
if [ $? -eq 0 ]; then
  echo "[OK] Security Group http-bia-sg foi criado"
  
  # Validar inbound rule para o security group 'http-bia-sg'
  ...
  # Validar outbound rule para o security group 'http-bia-sg'
  ...
  echo ">[ERRO] Não achei o security group http-bia-sg. Ele foi criado?"
fi
